...
【動作フロー（loginモード）】
  ...
  10. ウィンドウ最大化 → UIA経由で「パスキーなしで続行」リンクをクリック
...
def handle_passkey_dialog(hwnd: int) -> bool:
    """
    パスキー選択画面でウィンドウを最大化し、UIAutomation経由で
    「パスキーなしで続行」リンクの座標を取得してPostMessageでクリックする。
    """
    try:
        # ウィンドウを最大化して描画を安定させる
        ...
        # UIAutomation で「パスキーなしで続行」リンクを探す
        from pywinauto import Application
        app = Application(backend='uia').connect(handle=hwnd)
        wrapper = app.window(handle=hwnd)
        link = wrapper.child_window(title='パスキーなしで続行', found_index=0)
        rect = link.rectangle()
        screen_x = (rect.left + rect.right) // 2
        screen_y = (rect.top + rect.bottom) // 2
        log.info(f"「パスキーなしで続行」発見: screen({screen_x}, {screen_y})")

        # CEFクライアント座標に変換してPostMessageでクリック
        client_pt = win32gui.ScreenToClient(target, (screen_x, screen_y))
        cx, cy = client_pt
