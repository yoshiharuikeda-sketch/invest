...
- **`invest_morning_shutdown`（09:10）+ `invest_afternoon_login`（15:20）**: 寄付き後〜引け前の長いアイドル中に kabuStation のログインセッションが失効するのを避けるため、いったん終了して引け前に再ログインする設計。
  ...
  - **多層防御（2026-06-06 実装済み）**: 15:20への前倒しに加え、以下を実装。
    - **(a) パスキー処理の確実化**: `kabu_autologin.handle_passkey_dialog` を、UIA `InvokePattern` での「パスキーなしで続行」押下を**複数回リトライ**（`_try_invoke_skip`、8回×1.5秒、複数タイトルパターン対応）する方式に変更。確実方式が全滅した時だけ従来のフォールバック（SetFocus+Tab+Enter／オレンジ検出）に落ちる。
